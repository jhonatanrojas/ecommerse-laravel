...
@section('content')
<div class="mb-6">
    <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">Estatus de Envíos</h1>
    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Configura y administra los estatus del flujo de envíos.</p>
</div>
...
<div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
    <div class="lg:col-span-1">
        <div class="p-6 bg-white border border-gray-200 rounded-lg shadow-sm dark:bg-gray-800 dark:border-gray-700">
            ...
            <form method="POST" action="{{ route('admin.shipping-statuses.store') }}" class="space-y-4">
                @csrf
                <div>
                ...
                <div>
                    ...
                    <input type="text" id="slug" name="slug" value="{{ old('slug') }}" placeholder="ej: in_transit (opcional)" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                    @error('slug')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    ...
                    <input type="text" id="color" name="color" value="{{ old('color', '#3B82F6') }}" placeholder="#3B82F6" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                    @error('color')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    <label for="sort_order" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Orden</label>
                    <input type="number" id="sort_order" name="sort_order" value="{{ old('sort_order', 0) }}" min="0" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                    @error('sort_order')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                <div class="space-y-2">
                    <label class="inline-flex items-center">
                        <input type="checkbox" name="is_active" value="1" class="w-4 h-4 text-blue-600 rounded border-gray-300" {{ old('is_active', true) ? 'checked' : '' }}>
                        <span class="ml-2 text-sm text-gray-700 dark:text-gray-300">Activo</span>
                    </label>
                    <label class="inline-flex items-center">
                        <input type="checkbox" name="is_default" value="1" class="w-4 h-4 text-blue-600 rounded border-gray-300" {{ old('is_default') ? 'checked' : '' }}>
                ...
                <tbody>
                    @forelse($statuses as $status)
                        <tr class="border-b dark:border-gray-700">
                            ...
                            <td class="px-6 py-4">
                                @if($status->color)
                                    <span class="inline-flex items-center px-2 py-1 text-xs font-medium rounded" style="background-color: {{ $status->color }}22; color: {{ $status->color }};">
                                        <span class="w-3 h-3 rounded-full mr-1" style="background-color: {{ $status->color }};"></span>
                                        {{ $status->color }}
                                    </span>
                                @else
                            ...
                            <td class="px-6 py-4">
                                <span class="text-sm text-gray-700 dark:text-gray-300">{{ $status->sort_order }}</span>
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4 space-y-2">
                                <details>
                                    ...
                                    <form method="POST" action="{{ route('admin.shipping-statuses.update', $status->id) }}" class="mt-2 space-y-2">
                                        @csrf
                                        @method('PUT')
                                        ...
                                        <input type="number" name="sort_order" value="{{ $status->sort_order }}" min="0" class="w-full p-2 text-xs border rounded">
                                ...
                                <div class="flex items-center gap-3">
                                    <form method="POST" action="{{ route('admin.shipping-statuses.toggle', $status->id) }}">
                                        @csrf
                                        @method('PATCH')
                                    ...
                                    @if(!$status->is_default)
                                        <form method="POST" action="{{ route('admin.shipping-statuses.default', $status->id) }}">
                                            @csrf
                                            @method('PATCH')
                                    ...
                                    <form method="POST" action="{{ route('admin.shipping-statuses.destroy', $status->id) }}" onsubmit="return confirm('¿Eliminar este estatus?');">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-6 text-center text-gray-500">No hay estatus configurados.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
